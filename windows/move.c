...
int moveUp(int *arr, int n, int rot){
    ...
    for(int j=0;j<n;j++){
        for(int i=1;i<n;i++){
            //case 1 - if number is 0 continue
            if(arr(i,j,n)==0){
                continue;
            }
            //case 2 - when above number is same ADD
            ...
            // case 3 - when above number is 0 MOVE UP
            else if(arr(i-1,j,n)==0){
                arr(i-1,j,n) = arr(i,j,n);
                arr(i,j,n) = 0;
                legalMove = 1;
            }
            //case 4 - when above number is a non zero and not equal DO NOTHING
            else{
                continue;
            }
        }
